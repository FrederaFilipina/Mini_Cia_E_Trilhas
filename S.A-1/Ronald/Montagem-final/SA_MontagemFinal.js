...
function preencherSelectTrilhas() {
    const select = document.getElementById('trilha');
    const trilhasSalvas = localStorage.getItem('ListagemTrilhas');
    if (!trilhasSalvas) return;

    const listaTrilhas = JSON.parse(trilhasSalvas);
...
// Funções auxiliares fictícias
function usuarioLogado() {
    return JSON.parse(localStorage.getItem('usuarioLogado') || '{}');
}

function calcularIdade(dataNacimento) {
    const hoje = new Date();
    const nasc = new Date(dataNacimento);
...
function listarEventos() {
    // Se quiser exibir eventos depois
}

function preencherCamposUsuario() {
    // Se quiser preencher dados do usuário
}
...
document.getElementById('eventoForm').addEventListener('submit', function (event) {
    ...
    const trilhasSalvas = localStorage.getItem('ListagemTrilhas');
    const listaTrilhas = trilhasSalvas ? JSON.parse(trilhasSalvas) : [];
    ...
    const nome = usuarioLog.nomeCompleto?.trim() || "Desconhecido";
    const idade = calcularIdade(usuarioLog.dataNacimento);
    const sexo = usuarioLog.sexo || "Não informado";
    ...
    const trilhaValida = listaTrilhas.find(t => t.nome === trilha);
    if (!trilhaValida) {
        document.getElementById('mensagem').innerText = "Trilha selecionada não é válida.";
        return;
    }
...
document.addEventListener('DOMContentLoaded', preencherSelectTrilhas);
